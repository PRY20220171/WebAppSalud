<template>
  <div id="Bars" class="">
    <v-app-bar app color="white py-0" height="50px">
    ...
    <v-navigation-drawer v-model="drawer" absolute temporary >
      <v-list>
        ...
        <v-list-group v-for="item in menu"
         :value="false" :prepend-icon="item.name[1]" >
          <template v-slot:activator>
            <v-list-item-title>{{item.name[0]}}</v-list-item-title>
          </template>
            <v-list-item v-for="([title, icon, link], i) in item.submenus" :key="i" link  active-class="subcolor_0--text text--accent-4">
