<template>
    <v-container id="ResultadosIndex" class="fill-height" fluid>
        <v-row align="center" justify="center">
            <v-card class="px-5 pb-5 pt-6">
                ...
                <v-row>
                    <v-col>
                        <v-data-table :headers="Headers.base" :items="collection.items" :expanded.sync="expanded" :search="search" @click:row="clickRow">
                            <template v-slot:expanded-item="{ headers, item }">
                              <td :colspan="headers.length" class="py-2" >
                                <v-row class="align-items-center px-5">
                                    <v-col v-for="header in Headers.locationDetail" v-bind:key="header.value"> 
                                        <b>{{header.text}}:</b> 
...
<script src="../../../components/_indexes/ResultadosIndex.js">
</script>
<style scoped>
