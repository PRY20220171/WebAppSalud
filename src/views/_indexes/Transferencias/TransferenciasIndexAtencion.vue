    ...
    <v-dialog v-model="dialog" persistent max-width="600px">
      <v-card>
        <v-card-title class="mx-4 mt-1 main_color--text text--darken-1 align-end">
          <span class="text-h5">Añadir Transferencia</span>
        </v-card-title>
        <v-divider></v-divider>
        <v-card-text>
          <v-select :items="estados" label="estados" required v-model="Transferencia.estado"></v-select>

          <v-text-field name="fecTransferencia" label="fec. Transferencia" v-model="Transferencia.registro" type="date"
            color="main_color" hide-details="auto" />

          <v-text-field name="descripcion" label="descripcion" v-model="Transferencia.descripcion" type="text"
            color="main_color" hide-details="auto" />
        ...
        <v-card-actions>
          ...
          <v-btn color="blue darken-1" text @click="addTransferencia"> Save </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-container>
</template>
<script src="../../../components/_indexes/TransferenciasAtencionIndex.js">
</script>